...
const Countdown = () => {
  ...
  React.useEffect(() => {
    setInterval(() => {
      const then = moment("02 8 2022, 24:00", "MM DD YYYY, h:mm");
      const now = moment();
      if (then < now) {
        setTime({
          days: undefined,
        ...
        setRadius({
          days: mapNumber(day, 30, 0, 0, 360),
          hours: mapNumber(hour, 24, 0, 0, 360),
          minutes: mapNumber(minute, 60, 0, 0, 360),
          seconds: mapNumber(second, 60, 0, 0, 360),
        });
      }
    }, 1000);
  });
  ...
  return (
    <div className={style.main}>
      ...
      <p className={style.subtitle}>
        Join the RCK Community to discuss where the markets are heading.{" "}
      </p>
      ...
      <div className={style.infoContainer}>
        <div className={style.infoRow}>
          ...
          <div className={style.info}>
            ...
            <h4 className={style.infoData}>Feb 8, 2022 (WED 12:00 PM)</h4>
          </div>
        </div>
...
const SVGCircle = ({ radius, time }) => (
  <svg className={style.countdownsvg}>
    <path
      ...
      fill="none"
      stroke="#ffffff"
      stroke-width={time <= 1 || time == 59 ? "0" : "5"}
